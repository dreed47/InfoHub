...
// handler code. Generic JSON/HTTP helpers come from portal_shared.hpp. The
// Bambu Cloud/local status-presentation helpers (cloud_portal_presentation,
// append_cloud_status_fields, etc.) are defined in this TU (Phase 2,
// SetupPortal decoupling) — declared in portal_shared.hpp, gated behind
// CONFIG_INFOHUB_PLUGIN_PRINTER, since SetupPortal's own handle_root/
// handle_health still call them when the printer plugin is enabled.
...
esp_err_t PrinterPlugin::handle_enabled_post(httpd_req_t* request) {
  ...
  plugin->apply_enabled_state_to_ui(enabled, /*lock_timeout_ms=*/2000);
...
// ---------------------------------------------------------------------------
// Per-event audio settings for this plugin's 8 built-in AudioNotifier events
// (enable/disable, custom-WAV upload, clear-to-default). Moved here from
// SetupPortal — see PrinterPlugin::audio_event_enabled()/
// save_audio_event_enabled() in printer_plugin.cpp.
// ---------------------------------------------------------------------------

esp_err_t PrinterPlugin::handle_audio_event_post(httpd_req_t* request) {
  auto* plugin = static_cast<PrinterPlugin*>(request->user_ctx);
  if (plugin == nullptr) return ESP_FAIL;
  if (!plugin->setup_portal_->is_request_authorized(request)) {
    return plugin->setup_portal_->send_locked_response(request);
  ...
  ESP_RETURN_ON_ERROR(
      plugin->save_audio_event_enabled(static_cast<uint8_t>(event_idx), enabled),
      kTag, "save event enabled failed");
  plugin->audio_notifier_->set_event_enabled(static_cast<AudioNotifier::Event>(event_idx), enabled);
...
// WAV upload for a specific event  POST /api/plugins/printer/audio/upload?event=N
// WAV must be 16 kHz, 16-bit, mono PCM. Max body 32 KB + header.
esp_err_t PrinterPlugin::handle_audio_upload(httpd_req_t* request) {
  auto* plugin = static_cast<PrinterPlugin*>(request->user_ctx);
  if (plugin == nullptr) return ESP_FAIL;
  if (!plugin->setup_portal_->is_request_authorized(request)) {
    return plugin->setup_portal_->send_locked_response(request);
  ...
  const esp_err_t save_err = plugin->config_store_->save_audio_event_pcm(
  ...
  plugin->audio_notifier_->set_event_pcm(static_cast<AudioNotifier::Event>(event_idx),
                                         std::move(samples));

  // Persist the display name so it survives reboots.
  if (!display_name.empty()) {
    plugin->save_audio_event_display_name(static_cast<uint8_t>(event_idx), display_name);
  }
...
// Clear custom sound  POST /api/plugins/printer/audio/clear  {"event": N}
esp_err_t PrinterPlugin::handle_audio_clear(httpd_req_t* request) {
  auto* plugin = static_cast<PrinterPlugin*>(request->user_ctx);
  if (plugin == nullptr) return ESP_FAIL;
  if (!plugin->setup_portal_->is_request_authorized(request)) {
    return plugin->setup_portal_->send_locked_response(request);
  ...
  plugin->config_store_->clear_audio_event_pcm(static_cast<uint8_t>(event_idx));
  plugin->save_audio_event_display_name(static_cast<uint8_t>(event_idx), "");
  plugin->audio_notifier_->clear_event_pcm(static_cast<AudioNotifier::Event>(event_idx));
...
void PrinterPlugin::register_portal_routes(httpd_handle_t server) {
  ...
  static constexpr RouteEntry kRoutes[] = {
      ...
      {"/api/plugins/printer/audio/event", HTTP_POST, &PrinterPlugin::handle_audio_event_post},
      {"/api/plugins/printer/audio/upload", HTTP_POST, &PrinterPlugin::handle_audio_upload},
      {"/api/plugins/printer/audio/clear", HTTP_POST, &PrinterPlugin::handle_audio_clear},
  };
